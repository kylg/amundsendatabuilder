...
val tbl_groups = dbutils.widgets.get("tbl_groups")
...
def loadTableDefs(spark: SparkSession, groupNum:Int): List[(DataFrame, Int)] = {
  val db_tbls = spark.sharedState.externalCatalog.listDatabases().flatMap(db=>{
    val tbls = spark.sharedState.externalCatalog.listTables(db)
    tbls.map(tbl=>(db, tbl))
  })
  val tblGroups = db_tbls.zipWithIndex.groupBy(tbl_index => {tbl_index._2%groupNum}).map(g => {(g._1, g._2.map(db_index=>db_index._1))})
  tblGroups.map(groupPair => {
    val columnDefList = groupPair._2.flatMap(db_tbl => {
      loadTblDefsByDb(spark, db_tbl._1, db_tbl._2)
    })
    (spark.createDataFrame(columnDefList), groupPair._1)
  }).toList
}

def loadTblDefsByDb(spark: SparkSession, db:String, tbl:String):List[ColumnDef] = {
  import java.net.URI
  import java.text.SimpleDateFormat
  import java.util.Date
  var colDefList = List[ColumnDef]()
  try {
    ...
    colDefList = tblObj.dataSchema.fields.map(f => {
    ...
    case x: Exception => {
      ...
      println(x)
      colDefList = List(ColumnDef("", db, tbl, "", "", "", "", 0, "", "", false, false, "", 0, null, null, "", "", "", "", null, null, null, null, null, null))
    }
  }
  colDefList
}
...
// COMMAND ----------
val groupNum:Int = if(tbl_groups == null){
  6
}else{
  tbl_groups.toInt
}
